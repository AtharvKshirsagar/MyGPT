...
class Encoder:
    ...
    def __init__(self, encoder, bpe_merges):
        ...
        - '\'s' would split up things like Atharv's -> (Atharv, 's)
...
def get_encoder():
    ...
    cache_dir = os.path.join(home_dir, '.cache', 'mygpt')
...
if __name__ == '__main__':
    ...
    text = "Hello!! World. It's 2022. w00t :D 🤗"
    e = get_encoder()
    ...
    print(r['tokens'])
    # ['Hello', '!!', ' I', "'m", ' Atharv', ' K', '.', ' It', "'s", ' 2022', '.', ' w', '00', 't', ' :', 'D', ' 🤗']
    ...
    # {'token': ' Atharv', 'token_bytes': b' Atharv', 'token_translated': 'ĠAtharv', 'token_merged': ['ĠAndre', 'j'], 'token_ix': [10948, 73]}
    # {'token': ' K', 'token_bytes': b' K', 'token_translated': 'ĠK', 'token_merged': ['ĠK', 'arp', 'athy'], 'token_ix': [509, 5117, 10036]}
...
# Function to visualize BPE tree
def visualize_bpe_tree(bpe_merges):
    G = nx.Graph()
    for merge in bpe_merges:
        G.add_edge(merge[0], merge[1])
    pos = nx.spring_layout(G)
    nx.draw(G, pos, with_labels=True, font_weight='bold', node_size=700)
    plt.title('Byte Pair Encoder (BPE) Tree')
    plt.show()

# Function to visualize byte-to-unicode mapping
def visualize_byte_to_unicode(byte_encoder):
    bytes_list = list(byte_encoder.keys())
    unicode_list = list(byte_encoder.values())
    
    fig, ax = plt.subplots(figsize=(15, 5))
    ax.bar(bytes_list, [1] * len(bytes_list), color='blue', label='Byte')
    ax.bar(unicode_list, [1] * len(unicode_list), color='orange', label='Unicode')
    ax.set_xlabel('Byte / Unicode')
    ax.set_yticks([])  # Hide y-axis ticks
    plt.title('Byte-to-Unicode Mapping')
    plt.legend()
    plt.show()

# Example data (replace with actual data)
example_bpe_merges = list(zip(string.ascii_lowercase, string.ascii_lowercase[1:]))
example_byte_encoder = dict(enumerate(string.ascii_uppercase))

# Visualize BPE tree
visualize_bpe_tree(example_bpe_merges)

# Visualize byte-to-unicode mapping
visualize_byte_to_unicode(example_byte_encoder)
